...
public class SynBioHubCmd implements Callable<Integer> {
    ...
    protected boolean verifyInput() {
        if (username == null) {
            username = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Please enter your SynBioHub username (email address): ")));
        }

        if (password == null) {
            password = System.console().readPassword(CLIENT_LOG_PREFIX.concat("Please enter your SynBioHub password: "));
        }
        ...
        if (exclusiveUrlArgs.collectionUrl == null && exclusiveUrlArgs.serverUrl == null) {
            String response = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Do you want to upload SBOL files to an existing collection [Y | N]: "))).trim();

            while(!Y_N_PATTERN.matcher(response).matches()) {
                response = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Do you want to upload SBOL files to an existing collection [Y | N]: "))).trim();
            }

            if(response.equals("Y")) {
                exclusiveUrlArgs.collectionUrl = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Please enter the URL of the collection you wish to upload to: ")));
            } else {
                exclusiveUrlArgs.serverUrl = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Please enter the URL of the SynBioHub server you wish to upload to: ")));

                if (collectionName == null) {
                    collectionName = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Please enter a name for the new collection to create: ")));
                }
            }
        }

        if (dirPath == null) {
            dirPath = new String(System.console().readLine(CLIENT_LOG_PREFIX.concat("Please enter the path to the directory or SBOL file you wish to upload: ")));
        }
